...
const multipleRecordInsertion = async (req, res) => {
    try {
        ...
        const record = [];
        bodyData.map((each) => {
            const item = [id = each.id, first_name = each.first_name, last_name = each.last_name, email = each.email]
...
module.exports = { createSchema, insertRecord, getAllRecord, updateRecord, deleteParticularRecord, deleteAllRecord, multipleRecordInsertion }